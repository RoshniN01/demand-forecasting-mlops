...
The project also includes a monitoring layer using Prometheus and Grafana to observe the performance of the demand forecasting pipeline.

### Prometheus

Prometheus is used to collect and store monitoring metrics generated by the forecasting pipeline. The project exposes metrics related to model performance and pipeline execution, which can be queried using PromQL.

The Prometheus configuration is available at
